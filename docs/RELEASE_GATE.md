...
- `npm run release:gate` builds the mod, syntax-checks the relay/viewer harness, runs `git diff --check`, starts fresh local relays, and executes both the deterministic viewer smoke and a clean-profile host-to-viewer transport smoke. The host smoke compiles and runs the production C# `RelayClient`, `JsonHelper`, and `StateProtocol`; proves a new room can carry host capabilities and pawn state to a new viewer; proves a forged viewer identity is pinned; and checks correlated relay acceptance, host application, and disconnected-host failure. The browser smoke covers desktop, compact, and narrow sizes, claim/assignment, Armory search and Equip, visible command lifecycle states, reconnect/replay, command availability, and screenshot generation.
